...
import { File, MetadataCallback } from '@ionic-native/file/ngx';
...
export class FileTransferService {
   ...
   * @param callback 回调函数
   */
  fileUpload(url, fllePath, fileParams, callback: (res: any) => void, progressEvent?) {
   ...
   * 获取文件的大小
   */
  getFileSize(filePath, metadata: MetadataCallback) {
    ...
    this.fileDownload(fileTransfer, url, filePath, data => {
      ...
      if (data === '成功') {
        this.base64.encodeFile(filePath).then((base64File: string) => {
          ...
          this.base64ToGallery.base64ToGallery(base64File, { prefix: 'img_', mediaScanner: true}).then(
