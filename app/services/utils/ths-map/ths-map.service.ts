...
export class ThsMapService {
   ...
   */
  centerAndZoom(map, x: number, y: number, level?: number, wkid: number = 4326): void {
   ...
   */
  setExtent(map: any, xMin: number, yMin: number, xMax: number, yMax: number, wkid: number): void;

  setExtent(map: any, xMin: number | string | { graphics: {}[] }, yMin?: number, xMax?: number, yMax?: number, wkid: number = 4326): void {
    loadModules(['esri/geometry/Extent', 'esri/graphicsUtils']).then(([Extent, graphicsUtils]) => {
      let extent;
      if (typeof (xMin) === 'string') {
        const layer = map.getLayer(xMin);
        if (layer !== undefined) {
   ...
   */
  addGraphicsLayer(map, layerName: string, clickEvent?: (event: any) => void): Promise<any> {
  ...
  addMarker(x: number | {} | {}[], y?: number, graphicsLayer?: any, picUrl?: string, picWidth: number = 25,
            picHeight: number = 25, offsetX: number = 0, offsetY: number = 18.5, wkid: number = 4326,
   ...
   */
  clearLayer(map, layer: string | { clear(): void; }): {} {
    if (!layer) {
      return;
    }
    if (typeof (layer) === 'string') {
      const curLayer = map.getLayer(layer);
      if (curLayer) {
        curLayer.clear();
      }
    } else {
  ...
  getMapLevel() {
     ...
     * 渲染地图面
     */
    setMapProvinceface(map, graphicsLayer, geoJson, data, locate?) {
      loadModules(['jimu/jsonConverters', 'esri/graphic']).then(([jsonConverters, Graphic]) => {
          graphicsLayer.clear();
          const converter = new jsonConverters.geoJsonConverter();
          const jsonObj = converter.toEsri(geoJson);
          // countryInfo += jsonObjn.features;
          for (const item of jsonObj.features) {
            const aroundCode = item.attributes.Code || item.attributes.adcode.toString();
            if (aroundCode === data) {
              item.symbol = {
                color: [87, 107, 255, 30], // 必选，符号颜色
                outline: { // 可选，符号边线设置
                    color: [87, 107, 255], // 必选，边线颜色
                    width: 1, // 必选，边线宽度
                    type: 'esriSLS'
                },
              ...
              if (item.attributes.center) {
                this.centerAndZoom(map, item.attributes.center[0], item.attributes.center[1]);
                // this.addMarker(item.attributes.center[0], item.attributes.center[1], graphicsLayer, 'assets/img/map-zdhyqy.png')
              } else {
                this.centerAndZoom(map, item.attributes.centerX, item.attributes.centerY);
                // this.addMarker(item.attributes.centerX, item.attributes.centerY, graphicsLayer, 'assets/img/map-zdhyqy.png')
              }
              const polygonGraphic = new Graphic(item);
